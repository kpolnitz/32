...
Game::Game(int rows, int cols, int nFlatulans)
{
    ...
    // Create city
    m_city = new City(rows, cols); //this is why we did include
    
    // Add player
    int rPlayer = randInt(1, rows);
    int cPlayer = randInt(1, cols);
    m_city->addPlayer(rPlayer, cPlayer); //why we did include
    
    // Populate with Flatulans
    while (nFlatulans > 0)
    {
        int r = randInt(1, rows);   //why we did include 
...
void Game::play()
{
    ...
    {
        //made changes to look like the new window
        ...
        else
        {
            switch (action[0])
            {
                ...
                case 'r':
                    ...
                    break;
                case 'h':
                    m_city->history().display(); //giving problems
                    //access the history display, not game display
                    cout << "Press enter to continue.";
                    cin.ignore(10000,'\n');
                    //SHOULDNT COUNT AS A TURN!!
                    m_city->display();
                    continue;
            }
        }
