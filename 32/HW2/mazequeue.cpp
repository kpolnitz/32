...
bool pathExists(string maze[], int nRows, int nCols, int sr, int sc, int er, int ec){
    ...
    Coord current(r,c);
    int i = 1;
    while (aMaze.size() != 0) //out of moves
    {
        //keep adding to thes stack, analyze each point, and keep going until you get to the start
        Coord temp = aMaze.front(); //temp that looks at the coordinate
        r = temp.r();
        c = temp.c();
        ...
        if (maze[r-1][c] == '.')
        {
            maze[r-1][c] = '2'; //mark it so that it is no longer a dot, shows that the path has been found
            aMaze.push(Coord(r-1,c));
        }
