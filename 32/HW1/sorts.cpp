...
bool compareSensorPtr(const Sensor* lhs, const Sensor* rhs)
{
    ...
    //       compare two Sensors POINTED TO by lhs and rhs.
    ...
    return compareSensor(lhs[0], rhs[0]);
...
int main()
{
    ...
    // destruction time.
    {
        ...
        //first iterator, last iterator, means for comparison
        ...
        for (int j = 0; j < auxSensors.size(); j++)
            sensors[j] = *SensorPtrs[j];
