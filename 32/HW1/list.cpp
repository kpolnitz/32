...
{
    //path not in the subclasses string
    string name = c->name();
    string arrow = "=>";
   if(c->subclasses().empty())
   {
    ...
    string track =  path + name;
    //cout << path + name << arrow;
     //add to what was there before
    cout << track << endl;
    path = path + name + arrow;
    for (int a = 0; a < c->subclasses().size(); a++)
    {
        listAll(c->subclasses()[a], path); //pass a shorter and shorter piece
    }
